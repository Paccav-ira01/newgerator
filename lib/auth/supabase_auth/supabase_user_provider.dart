...
class MyEMAILSupabaseUser extends BaseAuthUser {
  MyEMAILSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> myEMAILSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = MyEMAILSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
